...
\begin{frame}{Matriz de correlación}
  ...
  \begin{itemize}
    \item Correlaciones moderadas: ninguna supera $|r| > 0.6$, descartando multicolinealidad severa.
    \item oldpeak y thalach son las más correlacionadas con el target.
  \end{itemize}
\end{frame}
...
\begin{frame}{Eliminación de outliers (IQR)}
  ...
  \begin{itemize}
    ...
    \item \textbf{Resultado:} 35 filas eliminadas (combinadas). Shape final: 268 filas $\times$ 22 columnas (tras one-hot).
...
\begin{frame}{Estandarización (StandardScaler)}
  ...
  \begin{block}{Resumen del preprocesamiento}
    ...
    \begin{tabular}{lcc}
      ...
      Eliminación outliers & 303 filas & 268 filas \\
...
\begin{frame}{PCA: Varianza explicada y Scree Plot}
  ...
  \begin{itemize}
    \item \textbf{Criterio de Kaiser} (eigenvalue $>1$): 7 componentes retenidos (59.1\% de varianza).
    \item \textbf{Umbral 80\%:} Se requieren 12 de 22 componentes -- estructura mayoritariamente NO lineal.
  \end{itemize}
\end{frame}
...
\begin{frame}{PCA: Loadings -- contribución de variables}
  ...
  \begin{itemize}
    \item \textbf{PC1:} Dominada por oldpeak, thalach, y variables de dolor torácico (cp).
    \item \textbf{PC2:} Pesos fuertes en colesterol (chol) y presión arterial (trestbps).
    \item Las variables one-hot del thal y slope tienen contribuciones relevantes en PC3--PC5.
  \end{itemize}
\end{frame}
...
\begin{frame}{PCA: Proyección 2D}
  ...
  \begin{itemize}
    \item Varianza explicada en 2D: $\approx 27\%$ (baja).
...
\begin{frame}{Resultados del clustering OPTICS}
  \begin{center}
    \begin{tabular}{lccc}
      \toprule
      \textbf{Proyección} & \textbf{Clusters} & \textbf{Ruido (\%)} & \textbf{Silhouette} \\
      \midrule
      PCA  & -- & -- & -- \\
      UMAP & -- & -- & -- \\
      \bottomrule
  ...
  \vspace{0.3cm}
  \begin{itemize}
    \item La proyección con mejor Silhouette determina cuál espacio es más adecuado para clustering.
    \item Los resultados son coherentes con las métricas de preservación topológica.
  \end{itemize}
\end{frame}
...
\begin{frame}{Resumen visual del flujo de trabajo}
  \begin{enumerate}
    ...
    \item \textbf{Clustering:} Visualización de clusters OPTICS sobre PCA y UMAP, reachability plots.
...
\begin{frame}{Interpretación crítica del análisis}
  ...
  \textbf{Limitaciones:}
  \begin{itemize}
    \item \textbf{Dimensionalidad:} Con solo $\approx 27\%$ de varianza en 2D, PCA pierde mucha información. Se usó 2D por visualización, no por optimalidad.
    \item \textbf{Tamaño muestral:} 268 registros es limitado para clustering basado en densidad.
    \item \textbf{UMAP es estocástico:} Los resultados pueden variar entre ejecuciones si no se fija \texttt{random\_state}.
    \item \textbf{Target no usado en clustering:} La evaluación se hace exclusivamente con métricas intrínsecas (Silhouette). No hay validación externa contra el ground truth de severidad.
  \end{itemize}
\end{frame}
...
\begin{frame}{Lecciones aprendidas}
  \begin{itemize}
    ...
    \item OPTICS sobre UMAP produce clusters más interpretables que sobre PCA para este dataset.
  \end{itemize}
  ...
  \begin{block}{Trabajo futuro}
    \begin{itemize}
      ...
      \item Incorporar validación externa con el target (ARI, NMI) como sanity check.
      \item Probar t-SNE como alternativa no lineal y comparar con UMAP.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Evaluación de la calidad del análisis}
  ...
  \begin{itemize}
    ...
    \item 35 outliers detectados y eliminados (11.5\% de las filas) con criterio IQR justificado.
  ...
  \begin{itemize}
    \item OPTICS configurado con parámetros estándar y documentados.
    \item Silhouette reportada (sin ruido). Reachability plots permiten inspección visual de la estructura de clusters.
  \end{itemize}
\end{frame}
